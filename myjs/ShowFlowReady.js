...
function showFlow(o) {
		...
		Ext.get(nodeArr[i].key).on('contextmenu', function(e) {
			...
			if (nodeType != 'start' && nodeType != 'end') {
				...
				if (nodeName == 'currentNode' || nodeName == 'publishNode') {
					...
					menu.add(new Ext.menu.Item({ text: '查看审批日记', url: url, handler: ShowNodeRemark }));
				} else {
					menu.add(new Ext.menu.Item({ text: '查看审批日记', disabled: true }));
				}
				
				//当前审批用户
				if (nodeName == 'currentNode') {
					var insMenu=new Ext.menu.Item( { text: '当前审批用户', menu: {items:[]} } );
					Ext.Ajax.request({
				...
				//已处理用户
				if (nodeName == 'currentNode' || nodeName == 'publishNode') {
					var EndUserMenu=new Ext.menu.Item( { text: '已处理用户', menu: {items:[]} } );
					Ext.Ajax.request({
						url: 'rule?wf_num=R_S003_B047',
						method:'POST',
						success : function(response, action) {
				...
				if (nodeType == 'innerChildFlow' || nodeType == 'outerChildFlow') {
					var insDocMenu = new Ext.menu.Item( { text: '查看子流程文档', menu: {items:[]} } );
	...
	for(var i = 0; i < linkArr.length; i++) {
		//连线
		connectTwoNode(linkArr[i].from, linkArr[i].to, linkArr[i].routerId);
...
function initNode() {
	...
	//修改当前审批节点的样式
	if (CurrentNodeid != '') {
		$(getJquerySelectorPrefix(CurrentNodeid)).css('background', getNodeBgFromHexColor('#ff0000'));
		$(getJquerySelectorPrefix(CurrentNodeid)).attr('name', 'currentNode');
	}
	
	//修改当前审批节点前已经审批完的节点样式
	var nodePath = getNodePath(EndNodeList), i;
	for (i = 0; i < nodePath.length; i++) {
		$(getJquerySelectorPrefix(nodePath[i])).css('background', getNodeBgFromHexColor('#ffcc00'));
		$(getJquerySelectorPrefix(nodePath[i])).attr('name', 'publishNode');
	}
}
