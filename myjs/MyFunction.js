...
function chooseNodeObjFromType(type, p) {
	...
	switch(type) {
		...
		case 'comm':
			...
			nodeObj.icon = '<i class="layui-icon layui-icon-username" style="font-size: 20px; color: #666666; position: absolute; right: 85px; margin-top: -15px;"></i>';
			break;
		case 'freedom':
			nodeObj.newId = getNextNodeId('T');
			...
			nodeObj.text = "自由活动";
			nodeObj.bgColor = '#ccab26';
			nodeObj.icon = '<i class="layui-icon layui-icon-set-sm" style="font-size: 20px; color: #666666; position: absolute; right: 85px; margin-top: -15px;"></i>';
			break;
		case 'gateWay':
...
function initFlowCharts(o) {
	...
	for(var i = 0; i < linkArr.length; i++) {
		...
		$('#' + linkArr[i].routerId).dblclick(function(event) {
			...
			connectionAttr($(this).context.id);
		});
...
function deleteNodeFromDB(idArr) {
	...
	for (i = 0; i < idArr.length; i++) {
		var url = "/bpm/rule?wf_num=R_S002_B011&Action=DeleteNode&Processid=" + processId + "&Nodeid=" + getRemovePrefixId(idArr[i]);
...
function SetProperty(a, b) {
	...
	var l = "/bpm/rule?wf_num=R_S002_B002&Processid=" + d + "&Nodeid=" + k + "&ExtNodeType=" + c + "&WF_Appid=" + top.GetUrlArg("WF_Appid");
